...
const capabilities = ["Built specifically for cleaning businesses", "Crew can see jobs from their own phone", "Recurring visits never get missed", "Customer notes and entry codes live in one place", "Payroll calculated from real clock time", "Invoicing tied directly to the completed job"] as const;
...
function Missing() { return <span aria-label="Not included" className="text-[var(--co-faint)]">—</span>; }
function Included() { return <span aria-label="Included" className="text-lg font-bold text-[var(--co-accent)]">✓</span>; }
  ...
  return (
    <main className="min-h-[100dvh] bg-[var(--co-bg)] text-[var(--co-ink)]">
      ...
      <section className="mx-auto grid max-w-6xl gap-12 px-4 pb-20 pt-14 sm:px-6 sm:pb-24 sm:pt-20 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:gap-16 lg:px-8 lg:pt-28">
        ...
        <div><ProductScreenshot src="/marketing/scheduling.jpg" alt="ServiceSpark staff calendar for a cleaning business" width={1568} height={744} priority /><p className="mt-5 text-sm font-medium leading-6 text-[var(--co-muted)]">Not a concept — this is the same system a real cleaning business runs its day-to-day operations on.</p></div>
      </section>

      <section className="border-y border-[var(--co-line-soft)] bg-[var(--co-surface)]"><div className="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-8"><h2 className="max-w-3xl text-3xl font-semibold tracking-[-0.025em] sm:text-4xl">Still running your business on pen and paper, spreadsheets, or Google Calendar?</h2><div className="mt-10 overflow-x-auto border border-[var(--co-line)]"><table className="min-w-[850px] w-full border-collapse text-sm"><thead><tr className="border-b border-[var(--co-line)] text-left"><th className="w-[30%] px-4 py-4 font-semibold">Capability</th>{["Pen & Paper", "Spreadsheets", "Google Calendar", "Generic scheduling software"].map((name) => <th key={name} className="px-3 py-4 text-center font-semibold text-[var(--co-muted)]">{name}</th>)}<th className="bg-[var(--co-accent-tint)] px-3 py-4 text-center font-semibold text-[var(--co-accent)]">ServiceSpark</th></tr></thead><tbody>{capabilities.map((capability) => <tr key={capability} className="border-b border-[var(--co-line-soft)] last:border-0"><th scope="row" className="px-4 py-4 text-left font-medium">{capability}</th><td className="px-3 py-4 text-center"><Missing /></td><td className="px-3 py-4 text-center"><Missing /></td><td className="px-3 py-4 text-center"><Missing /></td><td className="px-3 py-4 text-center"><Missing /></td><td className="bg-[var(--co-accent-tint)] px-3 py-4 text-center"><Included /></td></tr>)}</tbody></table></div></div></section>
      ...
      <section className="mx-auto max-w-6xl px-4 py-20 sm:px-6 lg:px-8"><div className="max-w-3xl"><h2 className="text-3xl font-semibold tracking-[-0.025em]">Built in the field, not the boardroom.</h2><p className="mt-5 leading-8 text-[var(--co-muted)]">ServiceSpark grew out of the day-to-day work of running a real cleaning business: moving schedules when a job changes, getting crews the details they need, keeping customer access information close to the work, and closing out completed jobs. It was built around those real operating needs, not an outsider&apos;s guess at what a cleaning crew needs.</p></div></section>

      <section className="border-y border-[var(--co-line-soft)] bg-[var(--co-surface)]"><div className="mx-auto grid max-w-6xl gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[0.7fr_1.3fr] lg:gap-16 lg:px-8"><div><h2 className="text-3xl font-semibold tracking-[-0.025em]">Questions, answered.</h2><p className="mt-3 leading-7 text-[var(--co-muted)]">A few practical details about using ServiceSpark.</p></div><MarketingFaq /></div></section>

      <section id="early-access" className="scroll-mt-6"><div className="mx-auto max-w-6xl px-4 py-20 sm:px-6 lg:px-8"><div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-start"><div><h2 className="text-3xl font-semibold tracking-[-0.025em]">A practical path to a calmer day.</h2><p className="mt-3 max-w-md leading-7 text-[var(--co-muted)]">Start with the information you already use, then keep the office and the field team in step.</p></div><ol className="divide-y divide-[var(--co-line-soft)] border-y border-[var(--co-line-soft)]">{steps.map((step, index) => <li key={step} className="flex gap-5 py-5"><span className="font-mono text-sm font-semibold text-[var(--co-accent)]">0{index + 1}</span><p className="font-medium leading-6">{step}</p></li>)}</ol></div><div className="mt-14 border-t border-[var(--co-line-soft)] pt-12"><div className="max-w-2xl"><h2 className="text-3xl font-semibold tracking-[-0.025em]">Ready to spend less time coordinating the work?</h2><p className="mt-3 leading-7 text-[var(--co-muted)]">Tell us a little about your business and we&apos;ll be in touch about early access.</p><h3 className="mt-8 text-xl font-semibold">Request early access</h3><p className="mt-2 leading-7 text-[var(--co-muted)]">We&apos;d love to learn how ServiceSpark could support your operation.</p></div><div className="mt-8 max-w-3xl"><LeadForm /></div></div></div></section>
