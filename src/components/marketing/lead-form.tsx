...
export default function LeadForm() {
  ...
  if (submitted) {
    return (
      <div
        ...
        role="status"
        tabIndex={-1}
        className="rounded-xl border border-[color-mix(in_srgb,var(--co-success)_35%,var(--co-line))] bg-[color-mix(in_srgb,var(--co-success)_8%,var(--co-surface))] p-6"
      >
        <h3 className="text-lg font-semibold text-[var(--co-ink)]">
  ...
  const inputClass = (name: FieldName) =>
    `co-input mt-1 w-full ${errors[name] ? "border-[var(--co-danger)]" : ""}`;

  return (
    <form
      noValidate
      onSubmit={handleSubmit}
      className="co-card grid gap-4 p-5 sm:grid-cols-2 sm:p-6"
    >
      <Field
        label="Business name"
        name="businessName"
      ...
      <Field label="Crew size" name="crewSize" required error={errors.crewSize}>
        <select
          id="crewSize"
          className={inputClass("crewSize")}
          value={form.crewSize}
          onChange={(event) => update("crewSize", event.target.value)}
          aria-invalid={Boolean(errors.crewSize)}
      ...
      <div className="sm:col-span-2">
        ...
        {serverError ? (
          <p role="alert" className="mb-3 text-sm text-[var(--co-danger)]">
            {serverError}
          </p>
        ) : null}
        <button
          type="submit"
          disabled={submitting}
          className="co-button-primary w-full sm:w-auto"
        >
...
function Field({
  ...
  return (
    <label
      className="block text-sm font-medium text-[var(--co-ink)]"
      htmlFor={name}
    >
      {label}
      {required ? " *" : ""}
      ...
      {error ? (
        <span
          id={`${name}-error`}
          role="alert"
          className="mt-1 block text-xs font-normal text-[var(--co-danger)]"
        >
          {error}
        </span>
      ) : null}
