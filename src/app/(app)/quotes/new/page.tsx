...
export default function NewQuotePage() {
  ...
  function addSelectedAddOnsToNotes() {
    ...
    const labels = ADD_ONS.filter((addOn) => selectedAddOns.includes(addOn.key)).map((addOn) => addOn.quantified ? `${addOn.label} × ${addOnQty[addOn.key] ?? 1}` : addOn.label);
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {ADD_ONS.map((addOn) => {
                  ...
                  return (
                    <span
                      key={addOn.key}
                      className="inline-flex items-center gap-1 rounded-full border border-[var(--co-line)] bg-[var(--co-surface)] p-0.5"
                    >
                      <button type="button" onClick={() => toggleAddOn(addOn.key)} className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium transition ${selected ? "bg-[var(--co-accent-fill)] text-white" : "text-[var(--co-muted)]"}`}>
                      ...
                      {selected && addOn.quantified ? <span className="flex items-center gap-1 pr-1.5"><button type="button" onClick={() => setQtyForAddOn(addOn.key, (addOnQty[addOn.key] ?? 1) - 1)} className="flex h-5 w-5 items-center justify-center rounded-full border border-[var(--co-line)]" aria-label={`Fewer ${addOn.label}`}><Minus className="h-3 w-3" aria-hidden /></button><span className="min-w-[1rem] text-center text-xs font-semibold">{addOnQty[addOn.key] ?? 1}</span><button type="button" onClick={() => setQtyForAddOn(addOn.key, (addOnQty[addOn.key] ?? 1) + 1)} className="flex h-5 w-5 items-center justify-center rounded-full border border-[var(--co-line)]" aria-label={`More ${addOn.label}`}><Plus className="h-3 w-3" aria-hidden /></button></span> : null}
                    </span>
                  );
                })}
              </div>
