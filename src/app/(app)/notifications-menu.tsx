...
export default function NotificationsMenu({ initialNotifications }: { initialNotifications: Notification[] }) {
  ...
  return <div ref={ref} className="relative">
    ...
    {open ? <div role="menu" aria-label="Notifications" className="absolute right-0 top-full z-30 mt-2 w-96 overflow-hidden rounded-xl border border-[var(--co-line-soft)] bg-white shadow-[0_8px_24px_rgba(18,24,19,0.12)]">
      ...
      <div className="max-h-[28rem] overflow-y-auto">
        {notifications.length ? notifications.map((notification) => {
          ...
          const className = `block border-b border-[var(--co-line-soft)] px-4 py-3 last:border-b-0 ${notification.readAt ? "bg-white" : "bg-[var(--co-accent-tint)]/45"}`;
