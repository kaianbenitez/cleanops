...
export default function NotificationsMenu({
  ...
  /** Renders the dropdown via createPortal + fixed positioning (same
   * pattern date-picker.tsx/calendar-filters-panel.tsx use) instead of an
   * absolute-positioned child. Needed only when the trigger sits inside a
   * clipping ancestor, like Calendar's overflow-hidden toolbar card — the
   * two original call sites (layout.tsx, app-nav.tsx) aren't, so they
   * default to false and render exactly as before. */
  portal?: boolean;
}) {
  ...
  const menuContent = (
    <div
      ...
      className={portal ? "co-date-popover fixed z-[60] w-96 overflow-hidden" : "absolute right-0 top-full z-30 mt-2 w-96 overflow-hidden rounded-xl border border-[var(--co-line-soft)] bg-[var(--co-surface)] shadow-[0_8px_24px_rgba(18,24,19,0.12)]"}
    >
  ...
  return <div ref={ref} className="relative">
    <button ref={triggerRef} type="button" onClick={() => setOpen((current) => !current)} aria-haspopup="menu" aria-expanded={open} aria-label={unreadCount ? `${unreadCount} unread notifications` : "Notifications"} className="relative rounded-lg p-2 text-[var(--co-muted)] transition-colors hover:bg-[var(--co-surface-muted)] hover:text-[var(--co-ink)]">
    ...
    {open ? (portal ? createPortal(menuContent, document.body) : menuContent) : null}
  </div>;
}
