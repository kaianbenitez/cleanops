...
export default function JobCard({ job, employees, draggable = false, onDragStart, onOpen, variant = "timeline", ordinalLabel }: { job: CardJob; employees: Employee[]; draggable?: boolean; onDragStart?: (event: React.DragEvent<HTMLAnchorElement>) => void; onOpen?: (jobId: string) => void; variant?: "timeline" | "list"; ordinalLabel?: string }) {
  ...
  return (
    <>
      ...
      >
        <div className="flex items-center justify-between gap-2 text-xs text-[var(--co-muted)]"><span className="flex items-center gap-1 text-sm font-bold tracking-[-0.01em] text-[var(--co-ink)]">{formatClockLabel(job.scheduledStartTime)}{ordinalLabel ? <span className="rounded bg-[var(--co-accent-tint)] px-1 py-0.5 text-[12px] font-semibold tracking-normal text-[var(--co-accent-text)]">{ordinalLabel}</span> : null}</span><span className="flex items-center gap-1.5 text-[var(--co-faint)]"><ClientHomeSymbols roomCounts={job.roomCounts} gateCodeOrKeyNotes={job.gateCodeOrKeyNotes} petNotes={job.petNotes} /><span className="rounded bg-[var(--co-surface)]/60 px-1.5 py-0.5 text-[12px]">{job.clientType === "commercial" ? "Commercial" : "Residential"}</span></span></div>
        ...
        <div className="mt-2 flex items-center justify-between gap-2 text-xs">
          ...
          <span className="flex shrink-0 items-center gap-1.5">
            {job.readiness ? <span className={`${readinessTone(job.readiness.primary)} rounded px-1.5 py-0.5 text-[12px] font-bold`} title={`${readinessReason(job.readiness)} · ${readinessAction(job.readiness)}${job.readiness.reasons.length ? ` · ${job.readiness.reasons.join("; ")}` : ""}`}>{job.readiness.primary}</span> : null}
