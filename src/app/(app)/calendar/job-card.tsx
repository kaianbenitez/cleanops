  ...
  return (
    <>
      ...
      >
        ...
        <p className={`mt-0.5 truncate text-[var(--co-muted)] ${isList ? "text-xs" : "text-[11px]"}`} title={formatCustomerAddress(job)}>{job.recurringSeriesId ? "↻ " : ""}{jobTypeLabel(job)} · {formatCustomerAddress(job)} · {formatEstimatedTime(job.estimatedDurationMinutes)}</p>
