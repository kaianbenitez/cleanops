...
export default function VerticalBoard({
  ...
  function dropOnEmployee(
    ...
    // Dragging within a column (sourceEmployeeId === employeeId) or from the
    // unassigned/no-time queue (no sourceEmployeeId) still adds the target
    // lane. Dragging card out of one column into another moves it — only
    // the lane it was dragged out of is replaced, so a multi-crew job (e.g.
    // Alice + Bob) dragged from Alice's column into Carol's becomes Bob +
    // Carol, not Alice + Bob + Carol.
    ...
    commitJobPatch(
      ...
      { employeeIds: nextEmployees, scheduledStartTime: nextTime ?? null },
      {
        ...
        onSuccess: () => {
          router.refresh();
          const targetEmployee = employees.find((entry) => entry.id === employeeId);
                ...
                return (
                  ...
                  >
                      ...
                      return (
                        ...
                        >
                          <JobCard
                            ...
                            onDragStart={(event) => {
                              event.dataTransfer.setData("text/plain", job.id);
                              event.dataTransfer.setData("application/x-cleanops-source-employee", employee.id);
                              event.dataTransfer.effectAllowed = "move";
                            }}
                            onOpen={setDetailJobId}
                            ordinalLabel={ordinalByJobId.get(job.id) ? ordinalLabel(ordinalByJobId.get(job.id)!) : undefined}
                          />
                    ...
                    {dragOver?.employeeId === employee.id ? (
                      <div
                        className="pointer-events-none absolute inset-x-2 z-20 -translate-y-1/2 rounded-lg border border-dashed border-[var(--co-accent-text)] bg-[var(--co-surface)]/90 p-2 text-center text-xs font-semibold text-[var(--co-accent-text)]"
                        style={{
