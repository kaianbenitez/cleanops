...
import { CalendarDays, Clock3, CreditCard, KeyRound, Mail, MapPin, NotebookText, Phone } from "lucide-react";
...
export function CustomerViewCards({
  ...
  const notes = [
    customer.generalNotes ? { icon: NotebookText, title: "General notes", text: customer.generalNotes } : null,
    schedulingPreference ? { icon: Clock3, title: "Scheduling preference", text: schedulingPreference } : null,
    accessNotes ? { icon: KeyRound, title: "Entrance & access instructions", text: accessNotes } : null,
  ].filter((note): note is { icon: typeof KeyRound; title: string; text: string } => Boolean(note));
      ...
      <section className="grid gap-6 xl:grid-cols-[305px_minmax(0,1fr)]">
        ...
        <div className="space-y-6">
          ...
          <Card title="Service notes & preferences" action={<button onClick={onEditFocus} className="text-sm font-semibold text-[var(--co-evergreen)] hover:underline">Edit</button>}>
            {notes.length ? <div className="grid gap-4 p-5 md:grid-cols-2">{notes.map((note) => <Preference key={note.title} icon={note.icon} title={note.title}>{note.text}</Preference>)}</div> : <p className="p-5 text-sm text-[var(--co-muted)]">No notes or preferences recorded.</p>}
          </Card>
        </div>
      </section>
    </div>
