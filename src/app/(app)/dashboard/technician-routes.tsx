...
export default async function TechnicianRoutes({ companyId, todayIso }: { companyId: string; todayIso: string }) {
  const [employees, rows] = await Promise.all([
    db.select({ id: users.id, firstName: users.firstName, lastName: users.lastName }).from(users).where(and(eq(users.companyId, companyId), eq(users.role, "employee"), eq(users.isActive, true))).orderBy(users.firstName),
