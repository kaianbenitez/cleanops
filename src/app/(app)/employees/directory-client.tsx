...
export default function EmployeeDirectory({ rows }: { rows: EmployeeDirectoryRow[] }) {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      const matchesFilter = filter === "All roles" || filter === "Active" ? (filter === "All roles" ? true : row.isActive) : filter === "Inactive" ? !row.isActive : row.isActive && row.status === filter;
  ...
  return <div className="space-y-5">
    <section className="grid gap-3 sm:grid-cols-3 xl:grid-cols-5">{[
      ["Active cleaners", `${active.length}/${rows.length}`, "Current roster"], ["Scheduled today", String(scheduled.length), "On the job board"], ["Available today", String(available.length), "Open for assignment"], ["Hours this week", totalHours.toFixed(1), "Clocked time"], ["Extras pending", money(totalExtras), "Tips + bonuses"],
