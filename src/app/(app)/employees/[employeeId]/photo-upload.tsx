...
export default function PhotoUpload({ employeeId, photoUrl, initials, onUploaded }: { employeeId: string; photoUrl: string | null; initials: string; onUploaded: () => Promise<void> }) {
  ...
  return <div className="relative h-[76px] w-[76px] shrink-0">
    ...
    <button type="button" onClick={() => input.current?.click()} disabled={busy} className="absolute -right-16 top-1 whitespace-nowrap rounded-full border border-[var(--co-accent-text)] bg-white px-2.5 py-1 text-[10px] font-bold text-[var(--co-accent-text)] shadow-sm">{busy ? "Uploading…" : "Upload photo"}</button>
