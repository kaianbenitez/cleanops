...
export default function PhotoUpload({ employeeId, photoUrl, initials, onUploaded }: { employeeId: string; photoUrl: string | null; initials: string; onUploaded: () => Promise<void> }) {
  ...
  return <div className="relative h-[76px] w-[76px] shrink-0">
    ...
    <button type="button" onClick={() => input.current?.click()} disabled={busy} className="absolute -bottom-2 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full border bg-white px-2 py-0.5 text-[9px] font-bold text-[var(--co-evergreen)]">{busy ? "Uploading" : "Upload photo"}</button>
