...
export async function GET(request: Request) {
  ...
  const [customerRows, invoiceRows, quoteRows, employeeRows] = await Promise.all([
    ...
    db.select({ id: users.id, firstName: users.firstName, lastName: users.lastName, title: users.title, phone: users.phone, email: users.contactEmail }).from(users).where(and(eq(users.companyId, admin.companyId), eq(users.role, "employee"), or(ilike(employeeFullName, term), ilike(users.firstName, term), ilike(users.lastName, term), ilike(users.title, term), ilike(users.phone, term), ilike(users.contactEmail, term)))).limit(5),
  ]);
