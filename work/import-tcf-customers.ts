  ...
  for (const row of customerRows) {
    ...
    const [customer] = await db
      ...
      .values({
        ...
        paymentMethods: s(row, "Payment Method") ? [s(row, "Payment Method")] : null,
        preferredDays: s(row, "Preferred Day") ? s(row, "Preferred Day").toLowerCase().split(/[,/&]+/).map((value) => value.trim()).filter(Boolean) : null,
        preferredTimeOfDay: /pm/i.test(s(row, "Preferred Time")) ? "PM" : /am/i.test(s(row, "Preferred Time")) ? "AM" : null,
        generalNotes: [s(row, "Notes"), s(row, "Important to you!"), s(row, "Don't CLEAN"), petNotes].filter(Boolean).join("\n\n") || null,
