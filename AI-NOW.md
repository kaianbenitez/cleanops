# Shimmer — current working memory

Last refreshed: 2026-08-18

## Current state

- This checkout's `main` is at `8413984` (asset cleanup for the Shimmer rebrand). The recent mainline work includes the My Day mobile-employee UX pass, the Shimmer code/legal/domain rebrand, and the final brand-asset cleanup.
- The Shimmer landing-page rebuild (motion-rich feature showcase, CTA/copy rework, hero swap) is merged into `main` as of commits `a743975`–`999015d`. A separate `cleanops-landing` worktree was noted in an earlier version of this file — confirm it's still active before assigning it new work; it may just be a stale reference now that the work is on `main`.
